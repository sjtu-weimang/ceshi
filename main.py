...
def uploadVideo():
    if request.method=="GET":
        res=make_response(render_template('video.html'))
        return res
...
def wav():
    ...
    if request.method == 'POST' and not text and not voiceid:
        return render_template('wav.html', taskid=taskid)
    ...
    # 开始上传，渲染模板
    if request.method == "GET" and not text:
        return make_response(render_template('wav.html', taskid=taskid))
    if not text:
        return make_response('Upload Error: No text upload')
    if not voiceid:
        voiceid = '1'
...
def video():
    taskid = request.values.get('taskid')
    if request.method == "GET":
        return render_template('video.html', taskid=taskid)
    if 'input_video' in request.files:
        objFile = request.files.get('input_video')
    else:
        return ('Upload Error: No such file')
